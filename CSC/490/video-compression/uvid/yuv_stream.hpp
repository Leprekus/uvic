...
class YUVStreamReader{
   private:
      u32 width, height;
   public:
      YUVStreamReader(std::istream& stream, unsigned int width, unsigned int height): 
         input_stream{stream}, active_frame{width,height}, frame_counter{0U},
      ...
      // read Y, Cb, Cr data from stdin into the active_frame
      ...
      void create_frame() {
         frame_buffer.push_back(YUVFrame420{width, height});
      }
      YUVFrame420 &get_frame() {
         return frame_buffer.at(frame_buffer.size() - 1);
      }

   private:
      // read a base * height bytes from stdin into vector A
      bool read_into_array(std::vector<unsigned char>& A){
         //input_stream.read(&A.at(0),A.size());
         //return (bool)input_stream;
         // subsequent reads overwrite previous values using constant space
         char c;
         unsigned int i = 0;
      ...
      std::vector<YUVFrame420> frame_buffer;
      unsigned int frame_counter;
};

class YUVStreamWriter{
   public:
      YUVStreamWriter(std::ostream& stream, unsigned int width, unsigned int height): 
         output_stream{stream}, active_frame{width,height}, frame_counter{0U},
         width{width}, height{height} {}
