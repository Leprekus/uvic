...
select ceil(reltuples * (select ret from selectivity)) as tuples
...
The optimizer prefers a sequential scan over indexes because the query tries to compute
a range. To retrieve all tuples from a single year in the range using an index, the 
DBMS needs to perform a tree traversal on the B+ tree to get to the index, get the 
memory locations of the pages containing the tuples, fetch all the pages and finally
repeat the steps for every year in the range. In contrast, A sequential scan
allows the DBMS to discard or output tuples on the fly as they are being processed.
Although the sequential scan may result in more reads, the DBMS can process these reads
faster as they can be processed in batches of large contiguous chunks during the sequential scan.
On the other hand, tracing pointers and fetching a large amount of non-contiguous pages often 
results in a bigger performance impact so the DBMS chooses the latter because it can be processed
faster.
